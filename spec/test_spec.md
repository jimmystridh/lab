...
This document specifies the test infrastructure for `lab`, including test-only command-line options and requirements for writing tests.
...
```bash
./lab --no-expand-tokens --and-exit exec 2>&1 | grep "{b}"
...
./lab --path=/tmp/test --and-exit --and-keys="beta" exec 2>&1

# Both inject keys AND render once (keys processed, then render, then exit)
./lab --and-exit --and-keys="query" exec
```
...
```bash
output=$(lab_run --path="$TEST_LABS" --and-keys="query"$'\r' exec 2>/dev/null)
...
```bash
lab_run --path="$TEST_LABS" --and-keys=$'\x1b' exec >/dev/null 2>&1
if [ $? -eq 1 ]; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys="beta" exec)
```

**RIGHT: Include terminating key**
```bash
# Ends with Enter
output=$(lab_run --path="$TEST_LABS" --and-keys="beta"$'\r' exec)
...
output=$(lab_run --and-exit exec)  # output will be empty!
```

**RIGHT: Capture stderr for rendered output**
```bash
output=$(lab_run --and-exit exec 2>&1)  # Redirect stderr to capture TUI
```
...
|----------|-------------|
| `LAB_WIDTH` | Override terminal width (columns) |
| `LAB_HEIGHT` | Override terminal height (rows) |
