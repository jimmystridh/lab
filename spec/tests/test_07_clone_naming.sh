...
output=$(lab_run --path="$TEST_LABS" exec clone https://github.com/user/myrepo.git 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec clone https://github.com/user/simple-repo 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec clone https://github.com/user/repo customname 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec clone https://github.com/user/testrepo 2>&1)
...
if echo "$output" | grep -q "git clone.*$TEST_LABS"; then
    pass
else
