...
output=$(lab_run --path="$TEST_LABS" exec clone git@github.com:user/myrepo 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec clone git@github.com:user/myrepo.git 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec clone https://gitlab.com/user/glrepo 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec clone git@gitlab.com:user/sshrepo 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec clone not-a-valid-uri 2>&1)
exit_code=$?
...
output=$(lab_run --path="$TEST_LABS" exec https://gitlab.com/user/repo 2>&1)
...
output=$(lab_run --path="$TEST_LABS" exec https://example.com/user/repo.git 2>&1)
if echo "$output" | grep -q "git clone"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" exec clone https://github.com/user/repo 2>&1)
