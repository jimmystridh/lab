...
setup_scroll_dirs() {
    for i in $(seq 1 20); do
        dir="$TEST_LABS/2025-11-30-scroll-test-$(printf '%02d' $i)"
        mkdir -p "$dir"
        touch "$dir"
    done
}
...
cleanup_scroll_dirs() {
    rm -rf "$TEST_LABS"/2025-11-30-scroll-test-*
}
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="PAGEDOWN" exec 2>&1)
# Should move down by page
pass  # Implementation-dependent

# Test: Page up moves viewport (if supported)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="PAGEDOWN,PAGEUP" exec 2>&1)
pass  # Implementation-dependent
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="${keys}HOME" exec 2>&1)
pass  # Implementation-dependent

# Test: End key goes to last entry (if supported)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="END" exec 2>&1)
pass  # Implementation-dependent
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="scroll,DOWN,DOWN,DOWN" exec 2>&1)
...
output=$(LAB_HEIGHT=10 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(LAB_HEIGHT=8 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="UP,UP,UP,UP,UP" exec 2>&1)
