...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="alp" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="alpha" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="gam" exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="xyz123notmatch" exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="BETA" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="bet" exec 2>&1)
if echo "$output" | strip_ansi | grep -qi "beta"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="project long" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="2025-11" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="alph" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="al" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="ab" exec 2>&1)
...
LONG_DIR="$TEST_LABS/2025-11-30-alphabetical-test-entry-name"
mkdir -p "$LONG_DIR"
touch "$LONG_DIR"
output=$(LAB_WIDTH=50 lab_run --path="$TEST_LABS" --and-exit --and-keys="alpha" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="a" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="test" exec 2>&1)
