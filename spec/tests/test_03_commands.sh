...
output=$(lab_run init 2>&1)
if echo "$output" | grep -q "lab()"; then
    pass
else
    fail "init should output shell function" "contains 'lab()'" "$output" "command_line.md#init"
fi

# Test: exec clone outputs git clone script
output=$(lab_run --path="$TEST_LABS" exec clone https://github.com/test/repo 2>&1)
...
output1=$(lab_run --path="$TEST_LABS" --and-keys=$'\r' exec 2>/dev/null)
output2=$(lab_run --path="$TEST_LABS" --and-keys=$'\r' exec cd 2>/dev/null)
