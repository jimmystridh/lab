...
output=$(cd "$FAKE_REPO" && lab_run --path="$TEST_LABS" exec worktree myfeature 2>&1)
...
PLAIN_DIR=$(mktemp -d)
output=$(cd "$PLAIN_DIR" && lab_run --path="$TEST_LABS" exec worktree plaindir 2>&1)
...
# Test: dot shorthand (lab . <name>) works like worktree
output=$(cd "$FAKE_REPO" && lab_run --path="$TEST_LABS" exec . dotfeature 2>&1)
if echo "$output" | grep -q "worktree add"; then
    pass
else
    fail "lab . <name> should emit git worktree add" "worktree add command" "$output" "command_line.md#worktree"
fi

# Test: bare dot (lab .) requires name argument
output=$(cd "$FAKE_REPO" && lab_run --path="$TEST_LABS" exec . 2>&1)
if echo "$output" | grep -qi "name"; then
    pass
else
    fail "lab . without name should show error" "error about name" "$output" "command_line.md#worktree"
fi
