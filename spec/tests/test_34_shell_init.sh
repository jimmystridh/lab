...
output=$(SHELL=/usr/local/bin/fish lab_run init "$TEST_LABS" 2>&1)
if echo "$output" | grep -q "function lab"; then
    pass
else
    fail "SHELL=fish should emit fish function" "function lab" "$output" "shell_init"
fi

# Test: SHELL=zsh emits bash/zsh function
output=$(SHELL=/bin/zsh lab_run init "$TEST_LABS" 2>&1)
if echo "$output" | grep -q "lab() {"; then
    pass
else
    fail "SHELL=zsh should emit bash/zsh function" "lab() {" "$output" "shell_init"
fi

# Test: --path with space form
INIT_DIR=$(mktemp -d)
output=$(lab_run --path "$INIT_DIR" --and-exit exec 2>&1)
exit_code=$?
...
output=$(lab_run --path="$INIT_DIR2" --and-exit exec 2>&1)
