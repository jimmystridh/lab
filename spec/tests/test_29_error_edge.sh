...
EMPTY_DIR=$(mktemp -d)
output=$(lab_run --path="$EMPTY_DIR" --and-exit exec 2>&1)
# Should handle empty directory gracefully
if echo "$output" | strip_ansi | grep -qiE "(search|lab|no|empty)"; then
    pass
else
...
output=$(lab_run --path="/nonexistent/path/12345" --and-exit exec 2>&1)
...
output=$(lab_run --path="$HIDDEN_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$FILES_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$SYMLINK_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$PERM_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$DEEP_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$SPECIAL_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$UNICODE_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$LONGNAME_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$MANY_DIR" --and-exit exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="INVALID_KEY" exec 2>&1)
...
# Test: HOME not set
output=$(HOME="" lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
# Should handle missing HOME
pass

# Test: TERM not set
output=$(TERM="" lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
