...
# Using --path should show alt-project, not directories from TEST_LABS
output=$(lab_run --path="$ALT_TRIES" --and-keys=$'\r' exec 2>/dev/null)
...
output=$(lab_run --path="$ALT_TRIES" --and-exit exec 2>&1)
if echo "$output" | grep -q "alt-project"; then
    pass
else
...
mkdir -p "$EMPTY_TRIES"
lab_run --path="$EMPTY_TRIES" --and-exit exec >/dev/null 2>&1
exit_code=$?
