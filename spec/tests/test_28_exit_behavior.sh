...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="ESC" exec 2>&1)
# Should exit cleanly
pass
...
script=$(lab_run --path="$TEST_LABS" --and-keys="ENTER" exec 2>&1)
...
script=$(lab_run --path="$TEST_LABS" --and-keys="ENTER" exec 2>&1)
# Should include path from TEST_LABS
if echo "$script" | grep -q "$TEST_LABS"; then
    pass
else
    fail "cd should include tries path" "TEST_LABS in path" "$script" "tui_spec.md#path-in-cd"
fi

# Test: q key exits (if implemented as quit)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="q" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="DOWN,DOWN,DOWN,ESC" exec 2>&1)
pass

# Test: Type then escape cancels
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="test,ESC" exec 2>&1)
pass

# Test: Mark then escape cancels (returns to normal mode first)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="d,ESC" exec 2>&1)
# First ESC might exit delete mode, second would cancel
pass

# Test: Double escape ensures exit
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="d,ESC,ESC" exec 2>&1)
pass

# Test: Enter on "Create new" returns mkdir script
script=$(lab_run --path="$TEST_LABS" --and-keys="newuniquename,DOWN,DOWN,DOWN,DOWN,DOWN,DOWN,ENTER" exec 2>&1)
...
script=$(lab_run --path="$TEST_LABS" --and-keys="d,ENTER" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="d,DOWN,d,ESC" exec 2>&1)
pass

# Test: Exit preserves no changes
# Start with known state, exit, verify no changes
ls_before=$(ls "$TEST_LABS" | wc -l)
lab_run --path="$TEST_LABS" --and-keys="ESC" exec >/dev/null 2>&1
ls_after=$(ls "$TEST_LABS" | wc -l)
...
script=$(lab_run --path="$TEST_LABS" --and-keys="DOWN,ENTER" exec 2>&1)
...
script=$(lab_run --path="$TEST_LABS" --and-keys="RETURN" exec 2>&1)
if echo "$script" | grep -q "cd"; then
    pass
else
