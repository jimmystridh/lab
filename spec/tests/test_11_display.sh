...
output=$(lab_run --path="$TEST_LABS" --and-exit --no-expand-tokens exec 2>&1)
...
if echo "$output" | grep -qE "(>|{section}|\*|→)"; then
    ...
    if echo "$output" | grep -q "$TEST_LABS"; then
        pass
    else
...
output_colors=$(lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
output_no_colors=$(lab_run --no-colors --path="$TEST_LABS" --and-exit exec 2>&1)
...
output_env=$( NO_COLOR=1 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
LONG_DIR="$TEST_LABS/2025-11-30-this-is-a-very-long-directory-name-for-testing"
mkdir -p "$LONG_DIR"
touch "$LONG_DIR"
output=$(lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
