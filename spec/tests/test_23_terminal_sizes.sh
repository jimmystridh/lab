...
output=$(LAB_WIDTH=80 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(LAB_WIDTH=120 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
...
output=$(LAB_WIDTH=40 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(LAB_WIDTH=30 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(LAB_WIDTH=20 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
# Should not crash
pass

# Test: Tall terminal (50 rows)
output=$(LAB_WIDTH=80 LAB_HEIGHT=50 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
...
output=$(LAB_WIDTH=80 LAB_HEIGHT=12 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(LAB_WIDTH=80 LAB_HEIGHT=8 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
# Should not crash, show minimal UI
pass

# Test: Very wide terminal (200 columns)
output=$(LAB_WIDTH=200 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(LAB_WIDTH=60 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
output=$(LAB_WIDTH=40 LAB_HEIGHT=40 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
if echo "$output" | strip_ansi | grep -qE "📁"; then
    pass
else
...
output=$(LAB_WIDTH=200 LAB_HEIGHT=10 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
if echo "$output" | strip_ansi | grep -qE "(Search|📁)"; then
    pass
else
...
output=$(LAB_WIDTH=20 LAB_HEIGHT=50 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
pass  # Should not crash

# Test: Header visible at various widths
for width in 40 60 80 100 120; do
    output=$(LAB_WIDTH=$width LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
    if echo "$output" | strip_ansi | grep -qi "lab"; then
        continue
    else
        fail "header should be visible at width $width" "Lab title" "$output" "tui_spec.md#header-visibility"
        break
    fi
done
pass
...
LONG_DIR="$TEST_LABS/2025-11-30-this-is-a-long-name-for-truncation-test"
mkdir -p "$LONG_DIR"
touch "$LONG_DIR"
output=$(LAB_WIDTH=60 LAB_HEIGHT=24 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
