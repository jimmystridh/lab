...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="mynewproject" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="testname" exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="newentry" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="unique12345,DOWN" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="mycustomname" exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="brandnewdir,DOWN,DOWN,DOWN,DOWN,DOWN" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="test-with_special.chars" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="$LONG_NAME" exec 2>&1)
