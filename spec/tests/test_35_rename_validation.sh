...
output=$(lab_run --path="$RVAL_DIR" --and-keys='CTRL-R,CTRL-A,CTRL-K,ENTER,ESC' exec 2>/dev/null)
...
output=$(lab_run --path="$RVAL_DIR" --and-keys='CTRL-R,CTRL-A,CTRL-K,TYPE=2025-11-01-existing,ENTER,ESC' exec 2>/dev/null)
if [ -z "$output" ] || ! echo "$output" | grep -q "mv"; then
    pass
else
...
output=$(lab_run --path="$RVAL_DIR" --and-keys='CTRL-R,CTRL-A,CTRL-K,TYPE=new name here,ENTER' exec 2>/dev/null)
...
output=$(lab_run --path="$RVAL_DIR" --and-keys='CTRL-R,ENTER' exec 2>/dev/null)
if [ -z "$output" ] || ! echo "$output" | grep -q "mv"; then
    pass
else
...
output=$(lab_run --path="$RVAL_DIR" --and-keys='CTRL-R,/,ENTER,ESC' exec 2>/dev/null)
if [ -z "$output" ] || ! echo "$output" | grep -q "mv"; then
    pass
else
