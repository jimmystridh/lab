...
# Note: Ctrl-C is \x03
lab_run --path="$TEST_LABS" --and-keys=$'\x03' exec >/dev/null 2>&1
exit_code=$?
...
output=$(lab_run --path="$TEST_LABS" --and-keys="xyz"$'\x7f\x7f\x7f'"beta"$'\r' exec 2>/dev/null)
...
output=$(lab_run --path="$TEST_LABS" --and-keys=$'\x1b[B\x1b[A\r' exec 2>/dev/null)
...
output=$(lab_run --path="$TEST_LABS" --and-keys=$'\x1b[B\x1b[B\x1b[A\r' exec 2>/dev/null)
if echo "$output" | grep -q "cd '"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys=$'\x0e\r' exec 2>/dev/null)
if echo "$output" | grep -q "cd '"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys=$'\x0e\x10\r' exec 2>/dev/null)
if echo "$output" | grep -q "cd '"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys="xyz"$'\x08\x08\x08'"beta"$'\r' exec 2>/dev/null)
if echo "$output" | grep -q "beta"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys="beta"$'\x01'"alpha"$'\r' exec 2>/dev/null)
...
output=$(lab_run --path="$TEST_LABS" --and-keys="alpha"$'\x01\x05'"beta"$'\r' exec 2>/dev/null)
if echo "$output" | grep -q "alphabeta"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys="betaa"$'\x02\x7f\r' exec 2>/dev/null)
if echo "$output" | grep -q "beta"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys="alpha"$'\x01\x06\x06\x06\x06\x06'"beta"$'\r' exec 2>/dev/null)
if echo "$output" | grep -q "alphabeta"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys="alphabeta"$'\x01\x06\x06\x06\x06\x06\x0b\r' exec 2>/dev/null)
...
output=$(lab_run --path="$TEST_LABS" --and-keys="hello"$'\x17'"beta"$'\r' exec 2>/dev/null)
if echo "$output" | grep -q "beta"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys="hello-world"$'\x17'"beta"$'\r' exec 2>/dev/null)
