...
if echo "$output" | strip_ansi | grep -qi "lab"; then
    ...
    fail "title should display in header" "Lab in header" "$output" "tui_spec.md#header-title"
fi
...
output=$(LAB_WIDTH=40 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
# Should not crash, may show abbreviated hints
pass

# Test: Header truncates gracefully
output=$(LAB_WIDTH=30 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
