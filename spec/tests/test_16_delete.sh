...
output=$(lab_run --path="$DEL_TEST_DIR" --and-keys='ESC' exec 2>/dev/null)
...
output=$(lab_run --path="$DEL_TEST_DIR" --and-keys='CTRL-D,ESC' exec 2>/dev/null)
...
output=$(lab_run --path="$DEL_TEST_DIR" --and-keys='CTRL-D,ENTER,Y,E,S,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "rm -rf"; then
    pass
else
...
output=$(lab_run --path="$DEL_TEST_DIR" --and-keys='CTRL-D,ENTER,n,o,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "rm -rf"; then
...
output=$(lab_run --path="$DEL_TEST_DIR" --and-keys='CTRL-D,DOWN,CTRL-D,ENTER,Y,E,S,ENTER' exec 2>/dev/null)
...
output=$(lab_run --path="$DEL_TEST_DIR" --and-keys='CTRL-D,CTRL-D,ESC' exec 2>/dev/null)
if echo "$output" | grep -q "rm -rf"; then
