...
VERY_LONG_DIR="$TEST_LABS/2025-11-30-this-is-an-extremely-long-directory-name-that-will-definitely-need-truncation"
...
LONG_DIR="$TEST_LABS/2025-11-30-moderately-long-directory-name-here"
mkdir -p "$LONG_DIR"
touch "$LONG_DIR"
output=$(lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
...
PARTIAL_META_DIR="$TEST_LABS/2025-11-30-this-is-a-very-long-directory-name-for-testing"
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="alph,ENTER" --no-expand-tokens exec 2>&1)
...
output=$(LAB_WIDTH=400 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
