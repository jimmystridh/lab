...
output=$(lab_run --path="$TEST_LABS" --and-keys="uniquequery"$'\r' exec 2>/dev/null)
...
if echo "$output" | grep -q "mkdir"; then
    ...
    output2=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="uniquequery" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-keys="newproject"$'\r' exec 2>/dev/null)
if echo "$output" | grep -q "mkdir"; then
    pass
else
