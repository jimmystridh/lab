...
LONG_DIR="$TEST_LABS/2025-11-30-this-is-a-very-long-directory-name-that-needs-truncation-testing"
mkdir -p "$LONG_DIR"
touch "$LONG_DIR"
# Navigate to the long entry and check truncation
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="long" exec 2>&1)
...
output=$(LAB_HEIGHT=30 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
...
TRUNCATE_DIR="$TEST_LABS/2025-11-30-extremely-long-name-for-truncation-background-test-verification"
mkdir -p "$TRUNCATE_DIR"
touch "$TRUNCATE_DIR"
output=$(LAB_WIDTH=60 lab_run --path="$TEST_LABS" --and-exit --and-keys="extremely" exec 2>&1)
