...
title_line=$(echo "$output" | grep "Lab Directory Selection")
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="d" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="newtest" exec 2>&1)
if echo "$output" | grep -q "📂"; then
    pass
else
...
LONG_DIR="$TEST_LABS/2025-11-30-unicode-test-very-long-name-for-truncation"
mkdir -p "$LONG_DIR"
touch "$LONG_DIR"
output=$(LAB_WIDTH=60 lab_run --path="$TEST_LABS" --and-exit exec 2>&1)
if echo "$output" | grep -q "…"; then
    pass
else
...
CJK_DIR="$TEST_LABS/2025-11-30-测试目录"
...
output=$(lab_run --no-colors --path="$TEST_LABS" --and-exit exec 2>&1)
stripped=$(echo "$output" | strip_ansi)
if echo "$stripped" | grep -qE "(Search|📁|→)"; then
    pass
else
