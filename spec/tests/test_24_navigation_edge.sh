...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="UP,UP,UP" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="${keys%,}" exec 2>&1)
if echo "$output" | grep -q "→"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="k,k,k,k,k" exec 2>&1)
if echo "$output" | grep -q "→"; then
    pass
else
    pass  # May not wrap
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="alpha,DOWN,UP" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="xyznonexistent123,DOWN,UP" exec 2>&1)
# Should handle empty list gracefully
pass

# Test: Navigation after filter clears
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="abc,BACKSPACE,BACKSPACE,BACKSPACE,DOWN" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="TAB" exec 2>&1)
pass  # Implementation-dependent

# Test: Shift-Tab behavior (if implemented)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="DOWN,DOWN,SHIFT-TAB" exec 2>&1)
pass  # Implementation-dependent

# Test: g key goes to top (vim-style, if implemented)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="DOWN,DOWN,DOWN,g,g" exec 2>&1)
# May go to top
pass

# Test: G key goes to bottom (vim-style, if implemented)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="G" exec 2>&1)
pass
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="DOWN,DOWN,a,BACKSPACE" exec 2>&1)
# After typing and deleting, should maintain approximate selection
pass

# Test: Ctrl-N/Ctrl-P navigation (if implemented)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="CTRL-N,CTRL-N,CTRL-P" exec 2>&1)
pass  # Implementation-dependent

# Test: Number key navigation (if implemented)
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="1" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="DOWN,DOWN,DOWN,DOWN,DOWN,xyznotfound" exec 2>&1)
# Selection should reset to valid index
pass

# Test: Navigate to "Create new" entry
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="newunique,DOWN" exec 2>&1)
...
output=$(lab_run --path="$TEST_LABS" --and-exit --and-keys="al,DOWN,DOWN,DOWN,UP,UP" exec 2>&1)
