...
# Verify Ctrl-G promotes a lab to a project directory
...
output=$(lab_run --path="$GRAD_DIR" --and-keys='CTRL-G,ESC' exec 2>/dev/null)
...
output=$(lab_run --path="$GRAD_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "mv "; then
    pass
else
...
# Test: LAB_PROJECTS overrides destination directory
output=$(LAB_PROJECTS="$PROJ_DIR" lab_run --path="$GRAD_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "$PROJ_DIR"; then
    pass
else
    fail "LAB_PROJECTS should override destination" "$PROJ_DIR in output" "$output" "graduate"
fi

# Test: Default destination uses parent of LAB_PATH
output=$(lab_run --path="$GRAD_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
...
output=$(lab_run --path="$NO_DATE_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
...
output=$(lab_run --path="$WT_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
...
# Test: Symlink source path points to the original lab basename
output=$(lab_run --path="$GRAD_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
symlink_line=$(echo "$output" | grep "ln -s ")
if echo "$symlink_line" | grep -q "2025-06-01-my-experiment"; then
    pass
else
    fail "Symlink should be created at original lab path" "2025-06-01-my-experiment in ln -s" "$symlink_line" "graduate"
fi
...
output=$(lab_run --path="$GRAD_DIR" --and-keys='CTRL-G,CTRL-D' exec 2>/dev/null)
if echo "$output" | grep -q "mv "; then
...
DEST_DIR=$(mktemp -d)
output=$(lab_run --path="$EDIT_DIR" --and-keys="CTRL-G,CTRL-A,CTRL-K,TYPE=${DEST_DIR}/custom-name,ENTER" exec 2>/dev/null)
...
script=$(eval $LAB_CMD exec --path="$E2E_DIR" --and-keys="CTRL-G,CTRL-A,CTRL-K,TYPE=${E2E_DEST}/graduated-project,ENTER" 2>/dev/null)
...
output=$(lab_run --path="$E2E_DIR" --and-keys='ENTER' exec 2>/dev/null)
...
output=$(lab_run --path="$E2E_DIR" --and-exit exec 2>/dev/null)
...
output=$(lab_run --path="$REG_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "git worktree move"; then
...
output=$(lab_run --path="$MULTI_DIR" --and-keys='CTRL-G,ENTER' exec 2>/dev/null)
