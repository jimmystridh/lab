...
output=$(cd "$FAKE_GIT_REPO" && lab_run --path="$VER_DIR" exec . fresh-name 2>&1)
...
output=$(cd "$FAKE_GIT_REPO" && lab_run --path="$VER_DIR" exec . feature1 2>&1)
...
output=$(cd "$FAKE_GIT_REPO" && lab_run --path="$VER_DIR" exec . nonum 2>&1)
...
output=$(lab_run --path="$VER_DIR" exec worktree "$FAKE_GIT_REPO" wtname 2>&1)
...
output=$(cd "$FAKE_GIT_REPO" && lab_run --path="$VER_DIR" exec worktree dir cwdname 2>&1)
if echo "$output" | grep -q "worktree add"; then
    pass
else
