...
output=$(lab_run --path="$TEST_LABS" --and-keys='CTRL-J,ENTER' exec 2>/dev/null)
...
output=$(lab_run --path="$TEST_LABS" --and-keys='CTRL-J,CTRL-K,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "cd '"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys='CTRL-N,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "cd '"; then
    pass
else
...
output=$(lab_run --path="$TEST_LABS" --and-keys='CTRL-N,CTRL-P,ENTER' exec 2>/dev/null)
if echo "$output" | grep -q "cd '"; then
    pass
else
...
first=$(lab_run --path="$TEST_LABS" --and-keys='ENTER' exec 2>/dev/null | grep "^cd '" | head -1)
round_trip=$(lab_run --path="$TEST_LABS" --and-keys='CTRL-J,CTRL-K,ENTER' exec 2>/dev/null | grep "^cd '" | head -1)
...
first=$(lab_run --path="$TEST_LABS" --and-keys='ENTER' exec 2>/dev/null | grep "^cd '" | head -1)
round_trip=$(lab_run --path="$TEST_LABS" --and-keys='CTRL-N,CTRL-P,ENTER' exec 2>/dev/null | grep "^cd '" | head -1)
if [ "$first" = "$round_trip" ]; then
    pass
else
